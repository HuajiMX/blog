...
class OGP_Fetcher {
    ...
    public function fetch( $url ) {
        // URLのバリデーション
        if ( empty( $url ) ) {
            return new \WP_Error( 'empty_url', __( 'The URL is empty.', 'simple-link-embed' ) );
        }

        if ( ! filter_var( $url, FILTER_VALIDATE_URL ) ) {
            return new \WP_Error( 'invalid_url', __( 'Invalid URL.', 'simple-link-embed' ) );
        }
        ...
        if ( ! isset( $parsed['scheme'] ) || ! in_array( $parsed['scheme'], array( 'http', 'https' ), true ) ) {
            return new \WP_Error( 'invalid_scheme', __( 'The URL scheme is not allowed.', 'simple-link-embed' ) );
        }
        ...
        $host = trim( $host, '[]' );
        if ( empty( $host ) ) {
            return new \WP_Error( 'invalid_host', __( 'Invalid host.', 'simple-link-embed' ) );
        }

        // ローカルホストと内部アドレスをブロック
        $blocked_hosts = array( 'localhost', '127.0.0.1', '0.0.0.0', '::1', '169.254.169.254' );
        if ( in_array( $host, $blocked_hosts, true ) ) {
            return new \WP_Error( 'blocked_host', __( 'Access to this host is restricted.', 'simple-link-embed' ) );
        }
        ...
        if ( $ip !== false ) {
            if ( ! $this->is_public_ip_address( $ip ) ) {
                return new \WP_Error( 'private_ip_blocked', __( 'Access to private IP addresses is not allowed.', 'simple-link-embed' ) );
            }
        }
        ...
        $response = wp_safe_remote_get( $url, array(
            'timeout'             => 15,
            'sslverify'           => true,
            'user-agent'          => 'Mozilla/5.0 (compatible; SimpleLinkEmbed/1.0)',
        ...
        if ( $status_code !== 200 ) {
            ...
            return new \WP_Error( 'http_error', sprintf( __( 'HTTP error: %d', 'simple-link-embed' ), $status_code ) );
        }
    ...
    private function parse_html( $html, $url ) {
        ...
        if ( ! $loaded ) {
            ...
            return new \WP_Error( 'parse_error', __( 'Failed to parse the HTML document.', 'simple-link-embed' ) );
        }
        ...
        $ogp_data['favicon'] = $this->extract_favicon_url( $xpath, $url );
    ...
    private function is_incomplete_youtube_cache( $url, $cached_data ) {
        ...
        if ( $this->is_youtube_channel_url( $url ) && '' === $image ) {
            $fallback_descriptions = array(
                __( 'This is a YouTube channel page.', 'simple-link-embed' ),
                'YouTubeチャンネルページです。',
            );

            if ( in_array( $description, $fallback_descriptions, true ) ) {
                return true;
            }
        }
    ...
    /**
     * ページから favicon URL を抽出
     *
     * @param \DOMXPath $xpath XPathオブジェクト
     * @param string    $base_url ページURL
     * @return string favicon URL
     */
    private function extract_favicon_url( $xpath, $base_url ) {
        ...
        if ( $nodes instanceof \DOMNodeList ) {
            foreach ( $nodes as $node ) {
                ...
                if ( '' === $rel || '' === $href || 0 === strpos( $href, '#' ) || preg_match( '#^(data|javascript|mailto):#i', $href ) ) {
                    continue;
                }

                $priority = $this->get_favicon_rel_priority( preg_split( '/\s+/', $rel ) );
                if ( PHP_INT_MAX === $priority ) {
                    continue;
                }

                $candidate_url = $this->make_absolute_url( $href, $base_url );
                if ( ! $this->is_supported_remote_asset_url( $candidate_url ) ) {
                    continue;
                }
        ...
        return $this->build_default_favicon_url( $base_url );
    }

    /**
     * rel 属性から favicon 候補の優先度を判定
     *
     * @param array<int, string> $rel_tokens rel のトークン配列
     * @return int 優先度。候補外は PHP_INT_MAX
     */
    private function get_favicon_rel_priority( $rel_tokens ) {
        if ( ! is_array( $rel_tokens ) || empty( $rel_tokens ) ) {
            return PHP_INT_MAX;
        }

        $normalized_tokens = array_values(
            array_filter(
                array_map( 'strtolower', $rel_tokens ),
        ...
        if ( in_array( 'icon', $normalized_tokens, true ) ) {
            return 0;
        }

        if ( in_array( 'apple-touch-icon', $normalized_tokens, true ) || in_array( 'apple-touch-icon-precomposed', $normalized_tokens, true ) ) {
            return 1;
        }

        if ( in_array( 'mask-icon', $normalized_tokens, true ) ) {
            return 2;
        }
    ...
    /**
     * サポート対象のリモートアセット URL かどうか
     *
     * @param string $url URL
     * @return bool
     */
    private function is_supported_remote_asset_url( $url ) {
        ...
        return in_array( strtolower( (string) $parsed_url['scheme'] ), array( 'http', 'https' ), true );
    }

    /**
     * デフォルトの favicon URL を生成
     *
     * @param string $url URL
     * @return string favicon URL
     */
    private function build_default_favicon_url( $url ) {
        $origin = $this->get_url_origin( $url );
        if ( '' === $origin ) {
            return '';
        }
    ...
    private function fetch_youtube_data( $url ) {
        ...
        if ( ! is_wp_error( $response ) && 200 === wp_remote_retrieve_response_code( $response ) ) {
            ...
            if ( is_array( $oembed ) && ! empty( $oembed['title'] ) ) {
                ...
                if ( '' !== $author_name ) {
                    ...
                    $description = sprintf( __( 'A YouTube video from %s.', 'simple-link-embed' ), $author_name );
                }
                ...
                return array(
                    ...
                    'site_name'   => 'YouTube',
                    'domain'      => 'youtube.com',
                    'favicon'     => $this->build_default_favicon_url( $url ),
                );
            }
        }
        ...
        if ( '' !== $video_id ) {
            return array(
                'title'       => __( 'YouTube Video', 'simple-link-embed' ),
                'description' => __( 'This video can be played on YouTube.', 'simple-link-embed' ),
        ...
        if ( '' !== $channel_label ) {
            ...
            return array(
                'title'       => $title,
                'description' => __( 'This is a YouTube channel page.', 'simple-link-embed' ),
                'image'       => '',
                'url'         => $url,
                'site_name'   => 'YouTube',
                'domain'      => 'youtube.com',
    ...
    private function fetch_youtube_page_data( $url ) {
        ...
        if ( empty( $ogp_data['favicon'] ) ) {
            $ogp_data['favicon'] = $this->build_default_favicon_url( $url );
        }
    ...
    private function fetch_twitter_data( $url ) {
        ...
        if ( $is_tweet ) {
            return array(
                ...
                'title'       => sprintf( __( 'Post by %s / X', 'simple-link-embed' ), $username ),
                /* translators: %s: X username. */
                'description' => sprintf( __( 'A post by %s. View more details on X.', 'simple-link-embed' ), $username ),
                'image'       => '',
                'url'         => $url,
                'site_name'   => 'X (Twitter)',
                'domain'      => 'x.com',
            ...
            return array(
                /* translators: 1: X username, 2: X username reused inside the handle format. */
                'title'       => sprintf( __( '%1$s (@%2$s) / X', 'simple-link-embed' ), $username, $username ),
                /* translators: %s: X username. */
                'description' => sprintf( __( 'Profile of %s. Check the latest posts on X.', 'simple-link-embed' ), $username ),
                'image'       => '',
                'url'         => $url,
                'site_name'   => 'X (Twitter)',
                'domain'      => 'x.com',
